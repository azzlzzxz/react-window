 ...
 * @LastEditTime: 2022-06-19 22:33:36
 * @FilePath: /VirtualList/src/react-window/DynamicSizeList.js
 */
// createListComponent 为了方便代码复用，高阶函数组件
import createListComponent from "./createListComponent4";
...
const getItemMetaData = (props, index, instanceProps) => {
  ...
  if (index > lastMeasureIndex) {
    ...
    for (let i = lastMeasureIndex + 1; i <= index; i++) {
      // 动态高度，先取默认高度
      const size = itemSize ? itemSize(i) : DEFAULT_ESTIMATED_SIZE;
      // 保存条目的size，offset
